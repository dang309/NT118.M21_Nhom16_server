...
describe('Auth routes', () => {
  describe('POST /v1/auth/register', () => {
    ...
    test('should return 201 and successfully register user if request data is ok', async () => {
      ...
      expect(dbUser).toMatchObject({ username: newUser.name, email: newUser.email });
  ...
  describe('POST /v1/auth/login', () => {
    ...
    test('should return 401 error if there are no users with that email', async () => {
      ...
      expect(res.body).toEqual({ code: httpStatus.UNAUTHORIZED, message: 'Tên đăng nhập hoặc mật khẩu sai' });
    });
...
describe('Auth middleware', () => {
  ...
  test('should call next with unauthorized error if access token is not found in header', async () => {
    ...
    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({ statusCode: httpStatus.UNAUTHORIZED, message: 'Hãy xác thực!' })
    );
  });
